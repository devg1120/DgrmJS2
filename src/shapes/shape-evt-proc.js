...
export function shapeCreate(svg, canvasData, shapeData, shapeHtml, cons, onTextChange, settingsPnlCreateFn) {
	const el = svgEl('g', `${shapeHtml}
		...
		.map(cc => `<circle data-key="${cc[0]}" data-connect="${cc[1].dir}"	class="hovertrack" data-evt-index="2" r="6" cx="0" cy="0" style="transform: translate(${cc[1].position.x}px, ${cc[1].position.y}px);" />`)
		.join()}`);
